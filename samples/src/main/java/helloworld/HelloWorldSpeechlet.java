...
public class HelloWorldSpeechlet implements Speechlet {
    ...
    public void onSessionStarted(final SessionStartedRequest request, final Session session)
        ...
        session.setAttribute("number", rand.nextInt(100) + 1);
    }
    ...
    public SpeechletResponse onIntent(final IntentRequest request, final Session session)
        ...
        if ("HelloWorldIntent".equals(intentName)) {
            return getHelloResponse(request, session);
    ...
    private SpeechletResponse getGuessNumberResponse(final IntentRequest request, final Session session) {
        ...
        if (guess == answer) {
            String speechText = "Congratulations!!  You guessed it!";

            // Create the Simple card content.
            SimpleCard card = new SimpleCard();
            card.setTitle("Number Guess");
            card.setContent(speechText);
        ...
        log.info("NUMBER GUESS: {} was guessed; {} was the answer; should continue? {}",
     ...
     * @return SpeechletResponse spoken and visual response for the given intent
     */
    private SpeechletResponse getHelloResponse(final IntentRequest request, final Session session) {
        String speechText = "Hello " + session.getUser().getUserId();
        //request.getIntent().getSlot("foo");
